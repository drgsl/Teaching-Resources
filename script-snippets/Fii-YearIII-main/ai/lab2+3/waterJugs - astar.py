...
def a_star(rootNode, destinationNode, pathList=[]):
    ...
    while not is_empty(pathList):
        path = pop_smaller_path(pathList)

        print(path)
        if get_last_node(path) == destinationNode:
            return path

        childrens = get_childrens(get_last_node(path))
        childrens = remove_cycles(path, childrens)

        heuristic(path, childrens, pathList)
